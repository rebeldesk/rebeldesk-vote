/**
 * API Route para alteração obrigatória de senha.
 * 
 * POST: Altera a senha do usuário logado e remove o flag de forçar troca.
 */
...
const alterarSenhaObrigatoriaSchema = z.object({
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Verifica se realmente precisa trocar senha
    if (!session.user?.forcar_troca_senha) {
      return NextResponse.json(
        { error: 'Troca de senha não é obrigatória' },
        { status: 400 }
      );
    }

    const body = await request.json();
    const dados = alterarSenhaObrigatoriaSchema.parse(body);
    ...
    // Atualiza a senha e remove o flag de forçar troca
