/**
 * Script para executar a migration 003_add_permitir_alterar_voto.sql
 * 
 * Uso:
 *   node scripts/run-migration-permitir-alterar-voto.js
 * 
...
async function runMigration() {
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, '../supabase/migrations/003_add_permitir_alterar_voto.sql');
    ...
    console.log('   Campo permitir_alterar_voto adicionado à tabela votacoes');

    // Verifica se a coluna foi criada
    const checkQuery = `
      SELECT column_name, data_type, column_default 
      FROM information_schema.columns 
      WHERE table_name = 'votacoes' AND column_name = 'permitir_alterar_voto';
    `;
    ...
    if (error.message.includes('already exists')) {
      console.log('\n⚠️  A coluna permitir_alterar_voto já existe na tabela votacoes');
