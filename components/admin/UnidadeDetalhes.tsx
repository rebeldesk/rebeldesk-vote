...
export function UnidadeDetalhes({ unidade, todasUnidades, isStaff }: UnidadeDetalhesProps) {
  ...
  const [mostrarFormMorador, setMostrarFormMorador] = useState(false);
  const [moradorEditando, setMoradorEditando] = useState<string | null>(null);
  ...
  const handleCancelarFormVeiculo = () => {
    setMostrarFormVeiculo(false);
    setVeiculoEditando(null);
  };

  const handleMoradorSalvo = () => {
    setMostrarFormMorador(false);
    setMoradorEditando(null);
    router.refresh();
  };

  const handleEditarMorador = (moradorId: string) => {
    setMoradorEditando(moradorId);
    setMostrarFormMorador(true);
  };

  const handleCancelarFormMorador = () => {
    setMostrarFormMorador(false);
    setMoradorEditando(null);
  };
                  ...
                  {mostrarFormVeiculo && (
                    <div className="mb-5 p-4 border border-gray-200 rounded-lg">
                      <VeiculoForm
                        ...
                        onCancel={handleCancelarFormVeiculo}
                      />
                    </div>
                  )}
          ...
          },
          {
            ...
            content: (
              <div className="space-y-4">
                <div className="flex items-center justify-between mb-5">
                  <h3 className="text-lg font-medium leading-6 text-gray-900">Moradores</h3>
                  {isStaff && (
                    <button
                      onClick={() => {
                        setMoradorEditando(null);
                ...
                {mostrarFormMorador && (
                  <div className="mb-5 p-4 border border-gray-200 rounded-lg">
                    <MoradorForm
                      unidadeId={unidade.id}
                      moradorId={moradorEditando}
                      initialData={
                        moradorEditando
                          ? unidade.moradores.find((m) => m.id === moradorEditando) || undefined
                          : undefined
                      }
                      onSuccess={handleMoradorSalvo}
                      onCancel={handleCancelarFormMorador}
                    />
                  </div>
                )}
